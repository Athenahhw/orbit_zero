...
public class AirplaneWar extends JPanel implements ActionListener, KeyListener {
    ...
    private void updateGame() {
        ...
        for (int i = bullets.size() - 1; i >= 0; i--) {
            ...
            b.update();
            if (b.isOutOfBounds(screenWidth, screenHeight)) {
                bullets.remove(i);
            }
        }

        for (Asteroid a : asteroids) {
            a.update(screenWidth, screenHeight);
        }

        for (SmallAsteroid sa : smallAsteroids) {
            sa.update(screenWidth, screenHeight);
        }

        for (Star s : stars) {
            s.update(screenHeight, screenWidth);
        }
